...
public class VisitPage extends TemplatePage {

    private final MarketService service;
    private final MarketYaml yaml;

    protected VisitPage(String title, String target, int page, MarketService service, MarketYaml yaml) {
    ...
    public Map<Integer, ItemStack> getItemMap() {
        ...
        Transaction[] transactions = service.selectTransactionBySearch(target, (page - 1) * 45, page * 45);
        // 可以下一页
    ...
    public void refreshPage() {
        ...
        buttonMap.put(51, ((ButtonItemBuilder)yaml.getItemBuilder("star")).actionPlayerCommand("market star " + target).build());
        buttonMap.put(52, ((ButtonItemBuilder)yaml.getItemBuilder("send")).actionPlugin("send " + target).build());
    ...
    public ViewPage getNext() {
        if(next == null && page < 30){
            next = new VisitPage(title, target, page + 1, service, yaml);
            next.setPre(this);
        }
        return next;
    }
